...
// Local type definition for layout result
type LayoutResult = {
  lineCount: number
  height: number
}
...
// Helper: measure natural width of prepared text (infinite width layout)
function measureNaturalWidth(prepared: PreparedTextWithSegments): number {
  ...
  while (true) {
    const line = layoutNextLine(prepared, cursor, Number.POSITIVE_INFINITY)
    if (line === null) break
